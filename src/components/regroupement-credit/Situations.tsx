const situations = [
  {
    ...
    description2:
      "Vous pourrez ainsi souffler en fin de mois et disposer de votre budget selon votre goût en consacrant une plus grande part à votre épargne, vos dépenses d'équipement, de loisirs...",
  },
  {
    ...
    description2:
      "Certains établissements de regroupement de crédits accepteront d'inclure un nouvel emprunt dans votre rachat. L'ensemble de vos prêts seront ainsi regroupés et étirés dans le temps. Vous pourrez donc réaliser votre projet sans augmenter vos mensualités.",
  },
  {
    ...
    description2:
      "Une fois votre taux d'endettement réduit, les établissements bancaires seront garantis que vous pourrez supporter les nouvelles mensualités et accepteront de financer votre projet.",
  },
];

function Situations() {
  return (
    <section id="situation" className="flex flex-col px-2 lg:px-36 2xl:px-56">
      ...
      <ul className="flex flex-col lg:grid lg:grid-cols-2 lg:gap-8 justify-start gap-4 px-2 lg:px-0 pb-4">
        {situations.map((situation, index) => (
          <li
            key={index}
            className="relative p-[0.3rem] rounded-2xl bg-gradient-button-light"
          >
        ...
        <li className="relative p-1 rounded-2xl bg-gradient-button-light">
          <div className="flex flex-col bg-textColor lg:h-[20rem] rounded-xl p-4">
            <h3 className="font-semibold uppercase pb-10 text-lg after:absolute after:top-[9rem] lg:after:top-[5.5rem] after:left-5 after:w-10 after:h-[5px] after:bg-secondary">
              CAS 4 : mes revenus ont récemment baissé et je ne parviens plus à
              régler mes mensualités
            </h3>
            <p>
              L&apos;arrivée à la retraite, une maternité, ou un accident de la
              vie a modifié le montant de vos revenus et vous éprouvez désormais
              des difficultés à régler vos mensualités ? Si une trop grande
              partie de votre budget est engloutie dans le remboursement de vos
              emprunts, le rachat de crédits va vous aider à rééquilibrer votre
              budget. Cette opération peut en effet réduire drastiquement le
              montant de vos mensualités vous permettant ainsi de retrouver une
              sérénité financière.
            </p>
          </div>
        </li>
